...
module.exports = (function () {
    ...
    var adapter = {
        ...
        registerConnection: function (connection, collections, cb) {
            ...
            connection.transaction = new Transation(connection);

            _.each(collections, function (collection) {
                collection.transaction = connection.transaction;
            });
        ...
        connectConnection: function (conn, cb) {
            ...
            if (persistent && !connected) {
                connection.mssqlConnection = new mssql.Connection(utils.marshalConfig(connection.config), cb);
            } else if (!persistent && !connected) {
                if (!assigned) {
                    connection.mssqlConnection = [];
                }

                connection.mssqlConnection[uniqId] = new mssql.Connection(utils.marshalConfig(connection.config), function (err) {
                    cb(err, uniqId);
                });
        ...
        find: function (connection, collection, options, cb) {
            ...
            var _collection = connections[connection].collections[collection];

            if (_collection.transaction && _collection.transaction.transactioned) {
                var request = new mssql.Request(_collection.transaction._transaction);
